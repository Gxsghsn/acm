...
//	   p->weight = w;		            // 修改权值
    ...
    while (p->adjVex!=v2 && p!=NULL) p=p->nextarc;
    if(p!=NULL) p->weight=w;
}
...
{
	AdjListNetworkArc<WeightType> *p;
    cout << "共有" << vexNum << "个顶点，" << arcNum << "条边。" << endl;
...
template <class ElemType,class WeightType>
void DFSTraverse(const AdjListDirNetwork<ElemType,WeightType> &g)   //initialize
{
	int v;
...
template <class ElemType,class WeightType>
void DFS(const AdjListDirNetwork<ElemType,WeightType> &g, int v)
{
	ElemType e;
	g.SetTag(v, VISITED);
	g.GetElem(v, e);
...
//19
template <class ElemType,class WeightType>
int FindPathInDist(const AdjListDirNetwork<ElemType,WeightType> &g,int v,int curdist,int dist,int targ)
{
    if(curdist==dist && v==targ) {return 1;}
    if(curdist>=dist) return 0;
    ElemType e;
	g.SetTag(v, VISITED);
	g.GetElem(v, e);
//	Write<char>(e);
	for (int w = g.FirstAdjVex(v); w != -1; w = g.NextAdjVex(v, w)){
		if (g.GetTag(w) == UNVISITED)
			if(FindPathInDist(g,w,curdist+1,dist,targ))	//iterate
            {
                cout<<e<<endl;
//                Write<char>(e);
                return 1;
...
//14
template<class ElementType, class WeightType>
int findPath(const AdjListDirNetwork<ElementType,WeightType> &g,int v,int tar)
{
    if(v==tar) return 1;
    g.SetTag(v,VISITED);
    for(int w=g.FirstAdjVex(v);w!=-1;w=g.NextAdjVex(v,w)){
...
//29
template<class ElementType, class WeightType>
int dijkstra(const AdjListDirNetwork<ElementType,WeightType> &g,int v,int tar)
{
    int n=g.GetVexNum();
    int MAXNUM=99999;
    g.SetTag(v,VISITED);
    ...
    for(int now=v;;){
        mindist=MAXNUM;
        for(int w=g.FirstAdjVex(now);w!=-1;w=g.NextAdjVex(now,w)){
            if(dist[w]>g.GetWeight(now,w)+dist[now]){
                dist[w]=g.GetWeight(now,w)+dist[now];
            }
        }

        for(int i=0;i<n && g.GetTag(i)==VISITED;i++){
            for(int w=g.FirstAdjVex(i);w!=-1;w=g.NextAdjVex(i,w)){
                if(g.GetTag(w)==VISITED) {continue;}
                if(dist[w]<mindist){
                    mindist=dist[w];
                    minindex=w;
                }
        ...
        g.SetTag(minindex,VISITED);
        now=minindex;
        if(now==tar) return mindist;
    }
    return -1;
}
